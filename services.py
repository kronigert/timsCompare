...
class DataLoaderService:
    """
    Handles the business logic of finding, parsing, and processing data
    ...
    def load_dataset_from_folder(self, folder_path: str) -> Dataset:
        ...
        if not segment_elements:
            ...
            self._parse_and_populate_segment(new_segment, method_element, root.find('instrument'), scan_mode_map, polarity_map, folder_path, {})
            ...
            for seg_element in segment_elements:
                ...
                self._parse_and_populate_segment(new_segment, seg_element, root.find('instrument'), scan_mode_map, polarity_map, folder_path, last_segment_params)
                dataset.segments.append(new_segment)
                if end_time >= 0:
    ...
    def _parse_parameters_for_scope(self, method_scope_element: ET.Element, instrument_scope_element: Optional[ET.Element], param_info: List[Dict], ion_polarity: Optional[str]) -> Dict:
        ...
        def find_and_get_value(p_config: Dict, current_results: Dict) -> Optional[Any]:
            ...
            if full_config.get("is_polarity_dependent") and ion_polarity:
                for dep_element in search_root.iter('dependent'):
                    polarity_attr = dep_element.get('polarity')
                    if polarity_attr and polarity_attr.lower() == ion_polarity.lower():
                        target = dep_element.find(f".//*[@permname='{permname}']")
...
class PlottingService:
    ...
    def generate_plot_as_buffer(self, dataset: Dataset, width_px: int, height_px: int, bg_color: str = "#E4EFF7", for_report: bool = False, dpi: int = 100, show_filename: bool = True) -> Optional[io.BytesIO]:
        """
        Generates a plot for the dataset's active segment and returns it as a BytesIO buffer.
        """
        try:
    ...
    def _generate_pdf(self, dataset: Dataset, selected_segment_indices: List[int], params_to_include: List[Dict], 
        ...
        class ReportPDF(FPDF):
            ...
            def _get_line_count(self, text: str, width: float) -> int:
                """A simple way to calculate how many lines a text string will occupy."""
            ...
            def _get_row_height(self, row: pd.Series, param_w: float, value_w: float, line_h: float = 5) -> float:
                """Calculate the maximum height needed for a row with potential text wrapping."""
